 ...
 *
 */
public class GoogleClusterTask_machine_id_username_vms {
    ...
    HashMap<Long, Long> cloudlet_machine = new HashMap<>();
    HashMap<String, Vm> machine_username_vm = new HashMap<>();
    private final DatacenterBrokerDynamic broker0;

    public static void main(String[] args) throws IOException {
        new GoogleClusterTask_machine_id_username_vms();
    }

    private GoogleClusterTask_machine_id_username_vms() throws IOException {
    ...
    private Double vmDestruction(Vm vm) {
        if(simulation.clock() > vm.getSubmissionDelay()){
            ...
            return vm.getSubmissionDelay() + 100;
        }
    }
    ...
    private Cloudlet createCloudlet(final tracereader.google.TaskEvent event) {
        ...
        if(event.getMachineId()!=-1 && event.getType()==TaskEventType.SUBMIT) {
            ...
            System.out.println(event.getUserName() + "_" + event.getMachineId());

            if(machine_username_vm.containsKey(event.getUserName() + "_" + event.getMachineId())) {

                cloudlet.setVm(machine_username_vm.get(event.getUserName() + "_" + event.getMachineId()));

            } else {
                Vm vm = createVm(event.getUniqueTaskId());
                System.out.println("Create Vm  " + "000" + event.getMachineId() + "000");
                vm.setId(Long.parseLong("000" + event.getMachineId() + "000"));
