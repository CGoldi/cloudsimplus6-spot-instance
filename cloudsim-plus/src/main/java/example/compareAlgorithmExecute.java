 ...
 */
public class compareAlgorithmExecute {
    ...
    private final ArrayList<Map<String, Object>> spotListImport = new ArrayList<>();
    private final ArrayList<Map<String, Object>> onDemandImport = new ArrayList<>();
    public static void main(String[] args) throws IOException, ClassNotFoundException {
        new compareAlgorithmExecute();
    }

    private compareAlgorithmExecute() throws IOException, ClassNotFoundException {

        ArrayList<Map<String, Object>> loadedSpot;
        try (ObjectInputStream ois = new ObjectInputStream(new FileInputStream("spotInstances.dat"))) {
            loadedSpot = (ArrayList<Map<String, Object>>) ois.readObject();
        }
        spotListImport.addAll(loadedSpot);

        ArrayList<Map<String, Object>> loadedVms;
        try (ObjectInputStream ois = new ObjectInputStream(new FileInputStream("onDemandInstances.dat"))) {
            loadedVms = (ArrayList<Map<String, Object>>) ois.readObject();
        }
        onDemandImport.addAll(loadedVms);
        ...
        simulation.terminateAt(1500);

        final DynamicAllocation allocationDynamic = new DynamicAllocation();
        final DynamicAllocationHLEM allocationHLEM = new DynamicAllocationHLEM();
        final DynamicAllocationHLEMAdjusted allocationHLEMAdjusted = new DynamicAllocationHLEMAdjusted();
        datacenter0 = createDatacenter(simulation, hostList0, allocationHLEMAdjusted);
    ...
    private void onHostDeallocationListener0(VmHostEventInfo vmHostEventInfo) {
        if (simulation.clock() > 10 && broker0.getResubmittingList().size() > 0) {
            ((DatacenterBrokerDynamic) broker0).resubmitSomeVms(1);
        }
        if (simulation.clock() > 1000 && broker0.getResubmittingList().size() > 0) {
            ((DatacenterBrokerDynamic) broker0).resubmitSomeVms(10);
        }
    }
    ...
    private void createHostLists() {
        ...
        int smallHosts = 45; // ThreadLocalRandom.current().nextInt(20, 51);
        int mediumHosts = 40; // ThreadLocalRandom.current().nextInt(20, 51);
        int largeHosts = 35; // ThreadLocalRandom.current().nextInt(15, 41);
        int xlHosts = 20; // ThreadLocalRandom.current().nextInt(10, 31);

        hostList0 = new ArrayList<>(smallHosts + mediumHosts + largeHosts + xlHosts);

        for (int i = 0; i < smallHosts; i++) {
            Host host = createHost(16, 16384, 5000, 200000);
            hostList0.add(host);
        }
        for (int i = 0; i < mediumHosts; i++) {
            Host host = createHost(32, 32768, 10000, 400000);
            hostList0.add(host);
        }
        for (int i = 0; i < largeHosts; i++) {
            Host host = createHost(64, 65536, 20000, 800000);
            hostList0.add(host);
        }
        for (int i = 0; i < xlHosts; i++) {
            Host host = createHost(128, 65536 * 2, 20000 * 2, 800000 * 2);
            hostList0.add(host);
        }
    }
    ...
    private void createVms() {
        ...
        for (Map<String, Object> entry : spotListImport) {
            DynamicVm vm = createSpotVm((Double) entry.get("delay"), (int) entry.get("profile"));
            broker0.submitVm(vm);
            createAndSubmitCloudlets(broker0, vm, (int) entry.get("mips"), cloudletList0);
        }
        for (Map<String, Object> entry : onDemandImport) {
            DynamicVm vm = createOnDemandVm((Double) entry.get("delay"), (int) entry.get("profile"));
            broker0.submitVm(vm);
            createAndSubmitCloudlets(broker0, vm, (int) entry.get("mips"), cloudletList0);
        }
    }
     ...
     */
    private DynamicVm createSpotVm(double addedDelay, int vmProfile) {

        double delayTime = 0;

        Map<String, Integer> profile = vmProfiles.get(vmProfile);
        ...
        vm.setHibernationTimeLimit(1500);
        vm.setWaitingTime(1500);
    ...
    private DynamicVm createOnDemandVm(double addedDelay, int vmProfile) {

        double delayTime = 0;

        Map<String, Integer> profile = vmProfiles.get(ThreadLocalRandom.current().nextInt(0, vmProfiles.size()));

        final OnDemandInstance vm = new OnDemandInstance(1000, profile.get("pes"), true);
        vm.setRam(profile.get("ram")).setBw(profile.get("bw")).setSize(profile.get("storage"));
        vm.addOnHostDeallocationListener(this::onHostDeallocationListener0);
        vm.setSubmissionDelay(delayTime + addedDelay);
        vm.setPersistentRequest(false);
        vm.setWaitingTime(100);
...
//        new SpotVmTableBuilder(finishedSpot).save("test.csv");
    ...
    private void saveVms() throws IOException {
        ...
        List<DynamicVm> spotVms = new ArrayList<>(broker0.getVmCreatedList());
        ...
        new ExecutionTableBuilder(finishedSpot.get(0).getExecutionHistory()).createJSON(finishedSpot, broker0);
