...
import tracereader.google.MachineEventType;
import tracereader.google.TaskEvent;
import tracereader.google.TaskEventType;
import tracereader.google.TraceField;
...
public final class GoogleTaskEventsTraceReader extends GoogleTraceReaderAbstract<Cloudlet> {
    ...
    HashMap<Long, Cloudlet> cloudletHashMap;

    public HashMap<Long, Cloudlet> getCloudletHashMap() {
        return cloudletHashMap;
    }

    public void setCloudletHashMap(HashMap<Long, Cloudlet> cloudletHashMap) {
        this.cloudletHashMap = cloudletHashMap;
    }
    /**
        ...
        */
        event
            ...
            .setTaskIndex(FieldIndex.TASK_INDEX.getValue(this))
                // TODO: machine ID was added here ---------------------------------------------------------------------------------------------------------
    ...
    /* default */ boolean requestCloudletStatusChange(
        final BiFunction<HashMap<Long, Cloudlet>, Long, Optional<Cloudlet>> cloudletLookupFunction,
        final int tag)
    {
        ...
        return cloudletLookupFunction
                .apply(this.getCloudletHashMap(), taskEvent.getUniqueTaskId())
...
//        cloudletStatusChangeEvents.add(attrsChangeSimEvt);
    ...
    protected Cloudlet createCloudlet(final TaskEvent taskEvent) {
        ...
        //TODO: disabled the need of an utilizationmodel to speed up the simulation ------------------------------------------------------------------
...
//                "so that the UtilizationModelDynamic.maxResourceUtilization attribute can be changed when defined in the trace file.");
    ...
    private DatacenterBroker createBroker(final String username) {
        // TODO: changed to DatacenterBrokerDynamic ------------------------------------------------------------------------------
        return new DatacenterBrokerDynamic(simulation, "Broker_"+username);
    }
